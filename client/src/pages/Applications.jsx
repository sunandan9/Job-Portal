...
const Applications = () => {
    ...
    useEffect(() => {
        const loadApplications = async () => {
            if (!user) return;
            try {
                const apps = await mockApi.getApplications(user);
                const jobs = await mockApi.getJobs();
                
                const populatedApps = apps.map(app => ({
                    ...app,
                    job: jobs.find(j => j._id === app.jobId) || { title: 'Unknown Job', company: 'Unknown Company' }
                }));
    ...
    const markAsPlaced = async (app) => {
        ...
        try {
            await mockApi.createPlacement({
                studentId: app.studentId,
                studentName: app.studentName,
                jobId: app.jobId,
                jobTitle: app.job?.title,
                companyName: app.job?.company || 'Company',
                packageAmount: salary,
