...
const Home = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const companiesCount = users.filter(u => u.role === 'employer').length;
                ...
                accepted.forEach(app => {
                    ...
                    const companyName = job ? job.companyName : 'Unknown';
                ...
                if (user && user.role === 'student') {
                    const studentApps = apps.filter(a => a.studentId === user.id.toString());
    ...
    const downloadReport = () => {
        let csv = 'Placement Portal Report\n';
        csv += `Generated on: ${new Date().toLocaleString()}\n\n`;

        csv += '--- Summary ---\n';
        csv += `Total Placements,${stats.placements}\n`;
        csv += `Active Students,${stats.students}\n`;
        csv += `Partner Companies,${stats.companies}\n`;
        csv += `Placement Rate,${stats.rate}\n\n`;

        csv += '--- Placement Records ---\n';
        csv += 'Student Name,Job ID,Status,Applied Date\n';
        acceptedApps.forEach(app => {
            csv += `${app.studentName || 'N/A'},${app.jobId},${app.status},${new Date(app.createdAt).toLocaleDateString()}\n`;
        });

        csv += '\n--- All Students ---\n';
        csv += 'Name,Email,Role\n';
        (stats.studentsList || []).forEach(s => {
            csv += `${s.name || s.username},${s.email || 'N/A'},${s.role}\n`;
        });
        ...
        <div style={{ padding: '2rem 3.5rem', minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
            ...
            <header className="animate-fade-up" style={{ marginBottom: '3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                    <button className="primary" style={{ padding: '0.75rem 1.5rem', fontSize: '0.9rem' }} onClick={downloadReport}>Generate Report</button>
                </div>
            </header>
            ...
            {activeTab === 'Reports' && (
                <div className="premium-card animate-fade-up stagger-3" style={{ padding: '5rem 2rem', textAlign: 'center' }}>
                    ...
                    <button className="primary" style={{ padding: '1rem 2rem' }} onClick={downloadReport}>Download Institutional Report</button>
                </div>
            )}
        </div>
    );
